...
class Day19 : Day(19) {
    ...
    override fun part2(): Int {
        ...
        while (stack.isNotEmpty() && !stack.contains("e")) {
            ...
            val shortestTwo =
                reverseReplacements.map { applyReplacement(it, current) }.flatten().toSet().sortedBy { it.length }
                    .take(2)
                    .filter { !numberOfTransformationsMap.contains(it) }
                    .toList()
            stack.addAll(shortestTwo)
            levels.add(numberOfTransformationsMap[current]!!)
            shortestTwo.forEach {
                numberOfTransformationsMap.putIfAbsent(
                    it,
                    numberOfTransformationsMap[current]!! + 1
                )
            }
        }
        println(levels)
